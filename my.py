...
questions_list = [] 
questions_list.append(Question('Сколько месяцев в году содержат по 28 дней?', '12', '1', '2', '10'))
questions_list.append(Question('Кто спит с открытыми глазами?', 'Рыбы', 'Воробьи', 'Люди', 'Лошади'))
questions_list.append(Question('Уличный термометр показывает 15 градусов. Сколько градусов покажут два таких термометра?', '15', '30', '0', '-15'))
questions_list.append(Question('Какие существа жуют желудком?', 'Раки', 'Люди', 'Птицы', 'Псы'))
questions_list.append(Question('Какие часы показывают правильное время только два раза в сутки?', 'Сломанные', 'Песочные', 'Солнечные', 'Биг Бен'))
questions_list.append(Question('Что теряют космонавты во время полёта?', 'Вес', 'Память', 'Ключи от квартиры', 'Сознание'))
questions_list.append(Question('Что из этого изобрели раньше?', 'Часы', 'Противогаз', 'Телефон', 'Термометр'))
questions_list.append(Question('Сколько прапрабабушек может быть у человека?', '8', '6', '12', '11'))
questions_list.append(Question('Что может одновременно идти и лежать, стоять и висеть?', 'Часы', 'Поезд', 'Корабль', 'Машина'))
...
def check_answer():
    ...
    print((window.score/window.total)*100, "баллов из 100")
def next_question():
    window.total += 1
...
#Кто изобрёл консервную банку для консервирования в 1810 году? (питер дюран)
#Какова продолжительность жизни стрекозы?(24 часа)
#Сколько сердец у Осьминога? (три)
#Какая кость в скелете - самая длинная? Бедренная кость
#Какой герой стал мужем принцессы Фионы? Шрек
